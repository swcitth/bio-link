  ...
  return (
    <div className={`relative z-10 ${isCompact ? "pb-5" : "pb-20"}`} style={{ fontFamily: selectedFont }}>
      ...
      <div className={`flex flex-col items-center px-4 ${isCompact ? "-mt-7" : "-mt-12"} relative z-10`}>
        ...
        <div className={`w-full flex flex-col ${isCompact ? "gap-3 mt-5" : "gap-4 mt-8"}`}>
          ...
          {visibleLinks.map((link) => {
            ...
            if (link.icon === "Youtube") {
              return (
                <div key={link.id} className={`flex flex-col ${isCompact ? "gap-2 mb-2" : "gap-3 mb-4"} w-full`}>
                  {link.items && link.title && <h3 className={titleClass} style={{ color: design.textColor || "#000" }}>{link.title}</h3>}
                  {subItems.filter(item => item.isVisible !== false && item.visible !== false).map((item, idx) => {
                    ...
                    return (
                      <div key={item.id || idx} className="flex flex-col gap-1">
                        ...
                        {(item.name || item.title) && <p className={nameClass} style={{ color: design.textColor || "#000" }}>{item.name || item.title}</p>}
                      </div>
                    );
                  })}
                </div>
                        ...
                        <div className="w-full rounded-2xl overflow-hidden shadow-md bg-black relative" style={{ aspectRatio: '9/16', maxHeight: isCompact ? '480px' : '600px' }}>
                          {tiktokId ? (
                            ...
                            <div className={`w-full h-full flex flex-col items-center justify-center text-white text-center px-2 ${isCompact ? "min-h-[200px]" : "min-h-[300px]"}`}>
                              <TiktokIcon size={isCompact ? 32 : 40} className="mb-2 opacity-40" />
            ...
            if (link.icon === "Image") {
              return (
                <div key={link.id} className={`flex flex-col ${isCompact ? "gap-2 mb-2" : "gap-4 mb-4"} w-full`}>
                  {link.items && link.title && <h3 className={titleClass} style={{ color: design.textColor || "#000" }}>{link.title}</h3>}
                    ...
                    {subItems.filter(item => item.isVisible !== false && item.visible !== false).map((item, idx) => {
                      ...
                      return (
                        <WrapperTag key={item.id || idx} {...(itemUrl ? { href: itemUrl, target: "_blank", rel: "noopener noreferrer" } : {})} className={`flex flex-col group ${itemUrl ? "hover:opacity-90 cursor-pointer" : "cursor-default"}`} style={{ textDecoration: 'none' }}>
                          ...
                          <h4 className={`${isCompact ? "text-[13px]" : "text-sm"} font-bold px-1`} style={{ color: design.textColor || "#000" }}>{item.name || item.title || "สินค้า"}</h4>
                          {isCompact && <p className="text-[10px] opacity-70 px-1 mt-0.5" style={{ color: design.textColor }}>{item.description || "รายละเอียดสินค้า"}</p>}
                          {isCompact && <p className="text-[11px] font-bold px-1 mt-1" style={{ color: design.textColor }}>{item.price ? `${item.price} THB` : "100 THB"}</p>}
                        </WrapperTag>
                      );
                    })}
                  </div>
            ...
            // 4. Normal Link
            return (
              <div key={link.id} className="w-full mb-2 flex flex-col gap-2">
                {link.items && link.title && <h3 className={`${titleClass} mb-1`} style={{ color: design.textColor || "#000" }}>{link.title}</h3>}
                {subItems.filter(item => item.isVisible !== false && item.visible !== false).map((item, idx) => {
                  const itemUrl = item.url || item.link;
                  const WrapperTag = itemUrl ? "a" : "div";
                  ...
                  return (
                    <WrapperTag 
                      ...
                      style={{
                        ...
                        backgroundColor: design.btnStyle === "outline" ? "transparent" : (design.btnBgColor || "#ffffff"),
                        color: design.btnTextColor || "#000000",
                    ...
                    >
                      <div className={`${isCompact ? "w-8 h-8" : "w-10 h-10"} rounded-full flex items-center justify-center shrink-0`} style={{ background: "rgba(0,0,0,0.06)", color: design.btnTextColor }}>
                        {ItemIcon && <ItemIcon size={isCompact ? 16 : 20} />}
                      </div>
                      <span className={`flex-1 text-center ${isCompact ? "pr-8" : "pr-10"} font-bold`} style={{ fontSize: isCompact ? "13px" : "16px", color: design.btnTextColor }}>
        ...
        {!isCompact && (
          <p className="mt-14 text-sm font-medium opacity-60 text-center" style={{ color: design.textColor || "#94a3b8" }}>
