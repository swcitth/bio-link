        ...
        <table className="w-full text-left border-collapse whitespace-nowrap">
          <thead>
            <tr className="text-[11px] font-bold text-slate-400 border-b border-slate-100 uppercase tracking-widest">
              ...
              {/* 🛠️ เพิ่มหัวคอลัมน์ ล็อกอินล่าสุด */}
              <th className="pb-4 px-4">ล็อกอินล่าสุด</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {users.length === 0 ? (
              <tr>
                {/* 🛠️ แก้ colSpan จาก 6 เป็น 7 เพื่อให้ข้อความอยู่ตรงกลางพอดีเมื่อเพิ่มคอลัมน์ */}
              ...
              currentItems.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  {/* 🛠️ คอลัมน์ที่เพิ่มใหม่: ล็อกอินล่าสุด */}
                  <td className="py-4 px-4">
                    <div className="flex flex-col">
                      {/* อ้างอิงตัวแปร lastLoginDate (วันที่) และ lastLoginDaysAgo (xx วันที่แล้ว) */}
                      <span className="font-medium text-sm text-slate-800">{user.lastLoginDate || 'ไม่มีข้อมูล'}</span>
                      {user.lastLoginDaysAgo && (
                        <span className="text-xs font-medium text-red-500">{user.lastLoginDaysAgo}</span>
                      )}
                    </div>
                  </td>
